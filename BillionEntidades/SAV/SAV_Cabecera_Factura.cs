...
{
    public class SAV_Cabecera_Factura : Cls_Bil_Base
    {
        ...
        public List<SAV_Detalle_Factura> Detalle_Pases { get; set; }
        public List<SAV_Clientes> Detalle_Clientes { get; set; }
        public List<SAV_Detalle_Pases> Detalle_Factura { get; set; }
        public List<SAV_Detalle_Error> Detalle_Errores { get; set; }
        public List<SAV_Detalle_Servicios> Detalle_Servicios { get; set; }
        public List<SAV_Detalle_Ok> Detalle_Ok { get; set; }

        public SAV_Cabecera_Factura()
        {
            init();

            this.Detalle_Pases = new List<SAV_Detalle_Factura>();
            this.Detalle_Clientes = new List<SAV_Clientes>();
            this.Detalle_Factura = new List<SAV_Detalle_Pases>();
            this.Detalle_Errores = new List<SAV_Detalle_Error>();
            this.Detalle_Servicios = new List<SAV_Detalle_Servicios>();
            this.Detalle_Ok = new List<SAV_Detalle_Ok>();
        ...
        private Int64? Save_Factura(out string OnError)
        {
            ...
            var db = sql_puntero.ExecuteInsertUpdateDeleteReturn(sql_puntero.Conexion_Local, 8000, "Sav_Procesa_Factura", parametros, out OnError);
        ...
        public Int64? SaveTransaction(out string OnError)
        {
            ...
            try
            {
                ...
                if (!id.HasValue)
                {
                    OnError = "*** Error: al grabar factura de devolución de contenedores ****";
                    return 0;
                }
                ID = id.Value;
