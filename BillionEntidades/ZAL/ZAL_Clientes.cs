...
{
    public class ZAL_Clientes : Cls_Bil_Base
    {
        ...
        public static List<ZAL_Clientes> Buscador_Clientes(string filtro, out string OnError)
        {
            OnInit("N5");

            parametros.Clear();
            parametros.Add("filtro", filtro);
            return sql_puntero.ExecuteSelectControl<ZAL_Clientes>(nueva_conexion, 6000, "sav_cliente_informacion", parametros, out OnError);
        }

        public static List<ZAL_Clientes> Clientes_Todos(out string OnError)
        {
            OnInit("N5");

            parametros.Clear();
            return sql_puntero.ExecuteSelectControl<ZAL_Clientes>(nueva_conexion, 6000, "sav_cliente_todos", null, out OnError);
        }
        ...
        public bool PopulateMyData(out string OnError)
        {
            ...
            var t = sql_puntero.ExecuteSelectOnly<ZAL_Clientes>(nueva_conexion, 6000, "sav_cargar_cliente", parametros);
