...
function segSEGCIA(control) {
    if (control != null) {
        var ve = control.value;
        if (ve != '0000000000000') {
            document.getElementById('va_nivel_cia').textContent = '';
        }
        else {
            document.getElementById('va_nivel_cia').textContent = '* obligatorio';
        }
    }
}


function invokeJsonTransportAT(objeto, url) {
...
function setmailsAT(objeto)
{
    try {
        ...
        //alert(div.length);   quitar en produccion
        if (div.length) {
            for (i = 1; i <= 5; i++) {
