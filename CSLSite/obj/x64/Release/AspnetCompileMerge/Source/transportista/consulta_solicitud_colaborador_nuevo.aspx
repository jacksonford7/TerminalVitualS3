<%@ Page  Language="C#" AutoEventWireup="true" Title="Registra Huella y Foto/Creaciòn de Permiso"
         CodeBehind="consulta_solicitud_colaborador_nuevo.aspx.cs" Inherits="CSLSite.consulta_solicitud_colaborador_nuevo" %>
  ...
  <div class="seccion">
     ...
     <asp:Repeater id="rpDetalle" runat="server">
                 ...
                 <td style=" width:225px"><asp:Label runat="server"  Width="225px" ID="lblTipo" Text='<%#Eval("TIPOD")%>'></asp:Label></td>
                  ...
                  <td style=" width:150px"><%#Eval("TAG")%></td>
                  <td style=" width:60px; display:none"><asp:CheckBox runat="server" ForeColor="Red" Enabled="false" id="chkRevisado" Checked='<%#Eval("ESTADOCOL")%>'/></td>
...
//                }
        ...
        function CallHuella(user, cnn) {
            ...
            try // Exception handling
            {
                ...
                var result = myobj.Only_RegistraHuella(user, true, false, 0);
                lista = [];
                var tbl = document.getElementById('tabla');
                if (tbl.rows.length == 1) {
                    return false;
        ...
        function CallCamara(user, cnn) {
            ...
            {
                var myobj = new ActiveXObject("Z_OnlyControl_2018.UC_Biometric_2018");
                var result = myobj.Only_CapturaFoto(user, 0);
                lista = [];
                var tbl = document.getElementById('tabla');
                if (tbl.rows.length == 1) {
                    return false;
