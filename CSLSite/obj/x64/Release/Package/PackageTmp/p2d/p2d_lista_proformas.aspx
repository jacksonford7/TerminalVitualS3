<%@ Page Title="" Language="C#" MasterPageFile="~/site.Master" AutoEventWireup="true" CodeBehind="p2d_lista_proformas.aspx.cs" Inherits="CSLSite.backoffice.p2d_lista_proformas" %>
      ...
      <div class="mt-4">         
        <nav class="mt-4" aria-label="breadcrumb">
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page" id="sub_opcion" runat="server">LISTADO DE COTIZACIONES EMITIDAS</li>
          </ol>
        </nav>
      </div>
       ...
       <ContentTemplate>
             <script type="text/javascript">
              ...
              <div class="bokindetalle" style=" width:100%; overflow:auto">  
                      <asp:Repeater ID="grilla" runat="server"  onitemcommand="tablePagination_ItemCommand" >
                       <HeaderTemplate>
                       <table cellpadding="0" cellspacing="0" border="0" class="table table-bordered table-sm table-contecon" id="grilla" width="100%">
                           <thead>
                          <tr>
                            <th class="th-sm"># COTIZACIÓN</th>
                            <th class="th-sm">FECHA</th>
                            <th class="th-sm"># CARGA</th>
                            <th class="th-sm">ZONA</th>
                            <th class="th-sm">CIUDAD</th>
                            <th class="th-sm">DIRECCIÓN ENTREGA</th>
                            <th class="th-sm">TOTAL BULTOS</th>
                            <th class="th-sm">TOTAL PESO</th>
                            <th class="th-sm">TOTAL M3</th>
                            <th class="th-sm">TOTAL PAGAR</th>
                            <th class="th-sm">PRINT</th>
                            <th class="th-sm">ANULAR</th>
                          </tr>
                        </thead>
                        <tbody>
                        </HeaderTemplate>
                        <ItemTemplate>
                            <tr class="gradeC">
                                <td class="center hidden-phone"> <%#Eval("ID_PROFORMA")%></td>
                                <td class="center hidden-phone"> <%#Eval("FECHA_EMISION")%></td>
                                <td class="center hidden-phone"> <%#Eval("NUMERO_CARGA")%></td>
                                <td class="center hidden-phone"> <%#Eval("ZONA")%></td>
                                <td class="center hidden-phone"> <%#Eval("CIUDAD")%></td> 
                                <td class="center hidden-phone"> <%#Eval("DIRECCION")%></td> 
                                <td class="center hidden-phone"> <%#Eval("CANT_BULTOS")%></td>
                                <td class="center hidden-phone"> <%#DataBinder.Eval(Container.DataItem, "TOTAL_TN", "{0:N2}")%></td> 
                                <td class="center hidden-phone"> <%#DataBinder.Eval(Container.DataItem, "TOTAL_M3", "{0:N2}")%></td> 
                                <td class="center hidden-phone"> <%#DataBinder.Eval(Container.DataItem, "TOTAL_PAGAR", "{0:c}")%></td> 
                                <td class="center hidden-phone">
                                    <a href="../reportes/p2d_proforma_preview.aspx?id_comprobante=<%#securetext(Eval("ID_PROFORMA")) %>"  target="_blank"><button type="button" class="btn btn-primary"><i class="fa fa-print"></i> </button></a>
                                </td> 
                                <td>
                               <div class="tcomand">
                                   <asp:Button ID="BtnAnular"  
                                   OnClientClick="return confirm('Esta seguro que desea anular este registro?');" 
                                   CommandArgument ='<%# jsarguments( Eval("ID_PROFORMA"))%>'
                                   CommandName="update"
                                   runat="server" 
                                   Text='Anular'  
