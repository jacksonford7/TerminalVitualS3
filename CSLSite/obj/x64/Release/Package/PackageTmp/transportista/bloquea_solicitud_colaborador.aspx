<%@ Page  Language="C#" AutoEventWireup="true" Title="Bloquea Permiso de Acceso"
         CodeBehind="bloquea_solicitud_colaborador.aspx.cs" Inherits="CSLSite.bloquea_solicitud_colaborador" %>
<%@ Register assembly="AjaxControlToolkit" namespace="AjaxControlToolkit" tagprefix="asp" %>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head id="Head1" runat="server">
    <title>Bloquea Permiso de Acceso</title>
 ...
 <i class="ico-titulo-2"></i><h1>Confirmación de Pago/Creaciòn de Permiso</h1><br />
  ...
  <div class="seccion">
     ...
     <asp:Repeater id="rpDetalle" runat="server">
                 <HeaderTemplate>
                 <table id="tabla" cellspacing="1" cellpadding="1" class="tabRepeat" style=" width:2250px">
                 <thead>
                 <tr>
                 <th>Tipo</th>
                 <th>Permiso</th>
                 ...
                 <ItemTemplate>
                 <tr class="point" >
                 <td style=" width:200px"><asp:Label runat="server"  Width="200px" ID="lblTipo" Text='<%#Eval("TIPOD")%>'></asp:Label></td>
                 <td style=" width:225px"><%#Eval("PERMISO")%></td>
                  ...
                  <td style=" width:80px">
                    <a style=" width:80px; font-weight:bold;"  id="a1" class="topopup"  onclick="return valbloqueaPermiso('<%#Eval("NUMSOLICITUD")%>', '<%#Eval("CIPAS")%>', '<%#Eval("NOMBRE")%>', '<%#Eval("RUCCIPAS")%>');">
                    <i></i>BLOQUEAR</a> 
                  </td>
...
//                }
        ...
        function valbloqueaPermiso(numsolicitud, cedula, nombres, ruc) {
            if (!bloqueaPermiso(numsolicitud, cedula, nombres, ruc)) {
                return false;
            }
            else {
        ...
        function bloqueaPermiso(numsolicitud, cedula, nombres, ruc) {
            if (confirm('¿Esta seguro de bloquear el permiso de acceso temporal?') == false) {
                return false;
            }
            var pageUrl = '<%=ResolveUrl("bloquea_solicitud_colaborador.aspx")%>'
            var validametodo = false;
            var scodeuserac = "";
            $.ajax({
                type: "POST",
                url: pageUrl + "/IsAvailableRAC",
                data: '{numsolicitud: "' + numsolicitud + '", cedula: "' + cedula + '", conductor: "' + cedula + ' - ' + nombres + '", ruc: "' + ruc + '"  }',
