<%@ Page Title="Consulta Certificados" Language="C#" MasterPageFile="~/site.Master" 
    AutoEventWireup="true" CodeBehind="consulta_inspeccion_certificado.aspx.cs" Inherits="CSLSite.consulta_inspeccion_certificado" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="placebody" runat="server">
        ...
        <nav class="mt-4" aria-label="breadcrumb">
          <ol class="breadcrumb">
            <li class="breadcrumb-item" id="opcion_principal" runat="server"><a href="#">Inspección</a></li>
             <li class="breadcrumb-item active" aria-current="page" id="sub_opcion" runat="server">Informe de Inspección de Buques</li>
          </ol>
        </nav>
      </div>
		   ...
		   <div class="col-md-12 d-flex justify-content-center"> 
             ...
             <div class="booking" >
                 ...
                 <tr class="point" >
                  ...
                  <td><%#Eval("DESC_NAVE")%></td>
                  <td><%#Eval("REFERENCIA")%></td>
                  <td><%#Eval("LINEA")%></td>
                  <td><%#Eval("BANDERA")%></td>
                  <td><%#Eval("FECHA_INSPE")%></td>
                          <td>
                           <select id="sellg" onchange="setCbVal('<%#Eval("SECUENCIA")%>',this)" >
                               <option selected="selected" value="E">Inglés</option>
                               <option value="S" >Español</option>
                           </select>
                           <input id='<%#Eval("SECUENCIA")%>' type="hidden" />
                       </td>
                   <td>
                   <div class="tcomand">
                       <span class=" btn btn-link" onclick="mostrar('<%# securetext(Eval("ID")) %>','<%#Eval("SECUENCIA")%>')" >Ver</span>
                      <span  onclick="descarga('<%# securetext(Eval("ID")) %>','<%#Eval("SECUENCIA")%>')" 
    ...
    <script type="text/javascript">
        ...
        function descarga(id, secuencia) {
            var lz = "E";
            var url = '../handler/inspeccion.ashx?sid=';
          ...
          function mostrar(id, secuencia) {
            var lz = "E";
            var url = '../inspeccion_buques/certificado_inspeccion.aspx?sid=';
            var cc = document.getElementById(secuencia);
            if (cc) {
                if (cc.value) {
                    lz = cc.value;
