...
namespace CSLSite
{
    public class Seguridad
    {
        ...
        /// <param name="tipoCatalogo">Nombre del catálogo que quiere consultar</param>
        /// <returns>Detalle del catálogo</returns>
        public static HashSet<Tuple<string, string>> getDetalleCatalogosSeguridadApp(string tipoCatalogo)
        {
            var xlista = new HashSet<Tuple<string, string>>();
            foreach (var i in CLSDataSeguridad.returnDetalleCatalogosSeguridadApp(tConexion.Master, tipoCatalogo))
            {
                xlista.Add(Tuple.Create(i.Item1.ToUpper(), i.Item2.ToUpper()));
            }
        ...
        public static HashSet<Tuple<int, string>> getTipoUsuarios()
        {
            var xlista = new HashSet<Tuple<int, string>>();
            foreach (var i in CLSDataSeguridad.returnTipoUsuarios(tConexion.Master))
            {
                xlista.Add(Tuple.Create(int.Parse(i.Item1.ToString()), i.Item2.ToUpper()));
            }

            return xlista;
        }

        public static HashSet<Tuple<int, string>> getFiltros( int iduser)
        {
            var xlista = new HashSet<Tuple<int, string>>();
            foreach (var i in CLSDataSeguridad.returnFiltros (tConexion.Service, iduser))
            {
                xlista.Add(Tuple.Create(int.Parse(i.Item1.ToString()), i.Item2.ToUpper()));
            }
        ...
        public static HashSet<Tuple<int, string>> getFiltrosApp(int iduser)
        {
            var xlista = new HashSet<Tuple<int, string>>();
            foreach (var i in CLSDataSeguridad.returnFiltrosApp(tConexion.Service, iduser))
            {
                xlista.Add(Tuple.Create(int.Parse(i.Item1.ToString()), i.Item2.ToUpper()));
            }
        ...
        public static List<user_options> getOpcionesDisponibles(int iduser,int idFiltro)
        {
            var xlista = new List<user_options>();
            foreach (var i in CLSDataSeguridad.returnOpcionesDisppoibles(tConexion.Service, iduser, idFiltro))
            {
                xlista.Add(new user_options() { servicio = i.Item1, opcion = i.Item2, descripcion = i.Item3, IdFiltro = i.Item4 }); //Tuple.Create(int.Parse(i.Item1.ToString()), i.Item2.ToUpper()));
            }

            return xlista;
        }

        public static List<user_options> getOpcionesAsignadas(int iduser)
        {
            var xlista = new List<user_options>();
            foreach (var i in CLSDataSeguridad.returnOpcionesAsignadas(tConexion.Service, iduser))
            {
                xlista.Add(new user_options() { servicio = i.Item1, opcion = i.Item2, descripcion = i.Item3, IdFiltro = i.Item4 }); //Tuple.Create(int.Parse(i.Item1.ToString()), i.Item2.ToUpper()));
            }
        ...
        public UsuarioSeguridad consultarUsuarioPorId(int id)
        {
            try
            {
                ...
                foreach (var item in CLSDataSeguridad.ValorLecturas(tConexion.Master, "MA_SE_ConsultaUsuarioPorId_tmp", tComando.Procedure, new Dictionary<string, string>() { { "idUsuario", id.ToString() } }))
                {
                    ...
                    u.IdTipoUsuario = int.Parse(item[21].ToString());
        ...
        public List<UsuarioSeguridad> consultarUsuarios(string usuario, string nombreUsuario, string identificacionUsuario, string nombreEmpresa, string estado, string tipoUsuario, string identificacionEmpresa)
        {
            try
            {
                ...
                foreach (var item in CLSDataSeguridad.ValorLecturas(tConexion.Master, "MA_SE_ConsultaUsuarios_tmp", tComando.Procedure, new Dictionary<string, string>() { { "usuario", usuario }, { "nombreUsuario", nombreUsuario }, { "identificacionUsuario", identificacionUsuario }, { "nombreEmpresa", nombreEmpresa }, { "estado", estado }, { "tipoUsuario", tipoUsuario },  { "identificacionEmpresa", identificacionEmpresa } }))
                {
        ...
        public List<UsuarioSeguridad> consultarUsuariosApp(string usuario, string nombreUsuario, string identificacionUsuario, string nombreEmpresa, string estado, string tipoUsuario, string identificacionEmpresa)
        {
            try
            {
                ...
                List<UsuarioSeguridad> usuarios = new List<UsuarioSeguridad>();
                foreach (var item in CLSDataSeguridad.ValorLecturas(tConexion.Master, "MA_SE_ConsultaUsuarios_tmp_appcgsa", tComando.Procedure, new Dictionary<string, string>() { { "usuario", usuario }, { "nombreUsuario", nombreUsuario }, { "identificacionUsuario", identificacionUsuario }, { "nombreEmpresa", nombreEmpresa }, { "estado", estado }, { "tipoUsuario", tipoUsuario }, { "identificacionEmpresa", identificacionEmpresa } }))
                {
                    UsuarioSeguridad u = new UsuarioSeguridad();
                    u.idUsuario = int.Parse(item[0].ToString().Trim());
                    u.usuario = item[1].ToString().Trim();
                    ...
                    u.usuarioIdentificacion = item[3].ToString().Trim();
                    u.usuarioCorreo = item[4].ToString().Trim();
                    u.nombreEmpresa = string.Format("{0} - {1}", item[6].ToString().Trim(),item[5].ToString().Trim());
                    u.codigoEmpresa = item[6].ToString().Trim();
                    u.tipoUsuario = item[7].ToString().Trim();
                    u.desTipoUsuario = item[8].ToString().Trim();
                    u.estado = item[9].ToString().Trim();
        ...
        public List<UsuarioSeguridad> consultarUsuariosAgentesApp(string usuario, string nombreUsuario, string identificacionUsuario, string nombreEmpresa, string estado, string tipoUsuario, string identificacionEmpresa)
        {
            try
            {
                string resultado = string.Empty;
                List<UsuarioSeguridad> usuarios = new List<UsuarioSeguridad>();
                //var oLstUsuarios = CLSDataSeguridad.LlenaCombos(tConexion.Master, "MA_SE_ConsultaUsuarios_tmp_appcgsaAG", tComando.Procedure, new Dictionary<string, string>() { { "usuario", usuario }, { "nombreUsuario", nombreUsuario }, { "identificacionUsuario", identificacionUsuario }, { "nombreEmpresa", nombreEmpresa }, { "estado", estado }, { "tipoUsuario", tipoUsuario }, { "identificacionEmpresa", identificacionEmpresa } });
                var oLstUsuarios = CLSDataSeguridad.ValorLecturas(tConexion.Master, "MA_SE_ConsultaUsuarios_tmp_appcgsaAG", tComando.Procedure, new Dictionary<string, string>() { { "usuario", usuario }, { "nombreUsuario", nombreUsuario }, { "identificacionUsuario", identificacionUsuario }, { "nombreEmpresa", nombreEmpresa }, { "estado", estado }, { "tipoUsuario", tipoUsuario }, { "identificacionEmpresa", identificacionEmpresa } });
                foreach (var item in oLstUsuarios)
                {
                    UsuarioSeguridad u = new UsuarioSeguridad();
                    u.idUsuario = int.Parse(item[0].ToString().Trim());
                    u.usuario = item[1].ToString().Trim();
        ...
        public List<UsuarioSeguridad> consultarUsuariosUnicoApp(int IdUsuario)
        {
            try
            {
                ...
                List<UsuarioSeguridad> usuarios = new List<UsuarioSeguridad>();
                foreach (var item in CLSDataSeguridad.ValorLecturas(tConexion.Master, "MA_SE_ConsultaUsuarios_tmp_app", tComando.Procedure, new Dictionary<string, string>() { { "IdUsuario", IdUsuario.ToString().Trim() } }))
                {
                    UsuarioSeguridad u = new UsuarioSeguridad();
                    u.idUsuario = int.Parse(item[0].ToString().Trim());
                    u.usuario = item[1].ToString().Trim();
        ...
        public string GuardarOpcionesAsignadas(int idUsuario, int idOpcion, int idEmpresa, int idCorp, int idUsuarioOperacion, string loginNameOperacion)
        {
            try
            {
                string resultado = string.Empty;
                resultado = CLSDataSeguridad.ValorEscalar(tConexion.Master, "MA_UsuarioOpcionesIngresar", 
                    new Dictionary<string, string>() { { "IdUsuario", idUsuario.ToString() },
                        { "IdOpcion", idOpcion.ToString() },
                        { "IdEmpresa", idEmpresa.ToString() },
                        { "IdCorporacion", idUsuarioOperacion.ToString() },
                        { "idUsuarioOperacion", idUsuarioOperacion.ToString() },
                        { "loginNameOperacion", loginNameOperacion } }, tComando.Procedure);
        ...
        public string EliminarOpcionesAsignadas(int idUsuario, int idOpcion ,int idUsuarioOperacion, string loginNameOperacion)
        {
            try
            {
                string resultado = string.Empty;
                resultado = CLSDataSeguridad.ValorEscalar(tConexion.Master, "MA_UsuarioOpcionesEliminar",
                   new Dictionary<string, string>() { { "IdUsuario", idUsuario.ToString() },
                        { "IdOpcion", idOpcion.ToString() },
                        { "idUsuarioOperacion", idUsuarioOperacion.ToString() },
                        { "loginNameOperacion", loginNameOperacion } }, tComando.Procedure);
