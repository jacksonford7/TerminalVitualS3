...
namespace CSLSite
{
    public partial class VBS_BAN_LoadingStowage : System.Web.UI.Page
    {
        #region "Clases"
        usuario ClsUsuario;
        ...
        private void ConsultarDataStowage()
        {
            try
            {
                ...
                var Resultadb = BAN_LoadingStowage.ConsultarLista(txtNave.Text, int.Parse(cmbLinea.SelectedValue), out OError);
                if (Resultadb != null)
                {
                    //var ResultadoDet = BAN_Stowage_Plan_Det.ConsultarLista(Resultadb.idStowageCab, out OError);
                    ...
                    //    catch { }
                    //}
                    //Resultadb.oDetalle = ResultadoDet;
                    //objStowageCab = Resultadb;
                    //Session["Transaccion_BAN_Stowage_Plan_Cab" + this.hf_BrowserWindowName.Value] = objStowageCab;

                    //##########################################
                    // Asignar los datos agrupados al Repeater
                    //##########################################
                    var datosAgrupados = Resultadb.GroupBy(o => o.hold).Select(group => new
                                                        {
                                                            Hold = group.Key,
                    ...
                    //##########################################
                    var LinqQuery = from Tbl in Resultadb.Where(Tbl => !String.IsNullOrEmpty(Tbl.deck))
                                    select new
                                    {
                                        id = Tbl.id,
                                        fecha = Tbl.fecha,
                                        ...
                                        idHold = Tbl.idHold,
                                        Hold = Tbl.hold.Trim(),
                                        cargo = Tbl.cargo.Trim(),
                                        marca = Tbl.marca.Trim(),
                                        exportador = Tbl.exportador.Trim(),
                                        consignatario = Tbl.consignatario.Trim(),
                                        aisv = Tbl.aisv,
                                        destino = Tbl.destino
