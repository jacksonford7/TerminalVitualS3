    ...
    public class CslHelperServicios
    {
        ...
        public static List<contenedorBuque> consultaBuquesContenedor(string referencia, string idsuario)
        {
            ...
            foreach (var item in CLSDataCentroSolicitud.ValorLecturaConexion("CS_ConsultaBuquesContenedores", 
                tComando.Procedure, tConexion.Sca, new Dictionary<string, string>() { { "referen" +
                "" +
                "ciaC", referencia }, { "identificacionC", idsuario } }))
            {
                ...
                c.rucCliente = item[19].ToString().Trim();
                c.terceros = item[20].ToString().Trim();
                c.estado_fac = item[21].ToString().Trim();
                c.liquidacion = item[22].ToString().Trim();
                rs.Add(c);
            }
            return rs;
        }
