...
{
    public partial class asume_terceros_sav : System.Web.UI.Page
    {
        #region "Clases"
        ...
        private void Listado_Cargas()
        {
            try
            {
                ...
                var Resultado = PagoAsignado.ListaAsignacionSAV(ClsUsuario.loginname.Trim(), ClsUsuario.ruc.Trim(), null, null);

                if (Resultado != null)
                {
                    if (Resultado.Exitoso)
                    {
                        var LinqQuery = from Tbl in Resultado.Resultado.Where(Tbl => !String.IsNullOrEmpty(Tbl.container))
                                        select new
                                        {
            ...
            if (Response.IsClientConnected)
            {
                try
                {
                    ...
                    if (e.CommandName == "Remover")
                    {
                        ...
                        var Resultado = pago.BorrarAsignacionSAV();
                        if (Resultado.Exitoso)
                        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                Server.HtmlEncode(this.TXTCONTAINER.Text.Trim());
        ...
        {
            TXTCONTAINER.Text = TXTCONTAINER.Text.ToUpper();
            if (Response.IsClientConnected)
            {
                try
                {
                    ...
                    if (string.IsNullOrEmpty(this.TXTCONTAINER.Text))
                    {
                        this.Mostrar_Mensaje(1, string.Format("<i class='fa fa-warning'></i><b> Informativo! </b>Por favor ingresar el número de contenedor"));
                        this.TXTCONTAINER.Focus();
                        return;
                    }
                    ...
                    var ClsUsuario = usuario.Deserialize(HttpContext.Current.Session["control"].ToString());
                    PagoAsignado pago = new PagoAsignado(0,this.TXTCONTAINER.Text.Trim(),this.TXTCLIENTE_RUC.Text,TXTCLIENTE_ASUMIDO.Text, ClsUsuario.ruc, ClsUsuario.loginname);
                    var Resultado = pago.NuevaAsignacionSAV();
