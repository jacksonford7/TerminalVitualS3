...
namespace CSLSite.services
{
    public class dataServiceHelper
    {
        ...
        private static SqlConnection Conection(string name)
        {
        ...
        //devolver las tipo de carga
        public static IEnumerable<Tuple<string, string>> ReturnTipoCarga()
        {
            var xsalida = new HashSet<Tuple<string, string>>();
            var con = getConector();
            if (con == null)
            ...
            using (con)
            {
                ...
                com.CommandType = CommandType.StoredProcedure;
                com.CommandTimeout = 120;
                com.CommandText = "sp_get_tipocarga";
                try
                {
                    con.Open();
                    using (var reader = com.ExecuteReader())
                ...
                catch (SqlException ex)
                {
                    ...
                    log_csl.save_log<Exception>(new Exception(sb.ToString()), "dataServiceHelper", "ReturnTipoCarga", "null", "N4");
                    xsalida.Add(Tuple.Create("0", "Catalogo incompleto [set]"));
                    return xsalida;
                }
                finally
        ...
        public static string getAvisosCGSA(string formulario=null)
        {
            ...
            if (con == null)
            {
                return "El Sistema de Terminal Virtual, Contecon S.A <br/>le agradece su visita..";
            }
            using (con)
            {
                var com = con.CreateCommand();
                ...
                catch (SqlException ex)
                {
                    ...
                    return "Sistema de Terminal Virtual, Contecon S.A <br/>le agradece su visita..";
                }
                finally
                {
                    if (con.State == ConnectionState.Open)
        ...
        public static string validarAduDoc(string tipoD, string docu, string ruc)
        {
            string msg=string.Empty;
            bool xd = false;
            if (!jAisvContainer.DaeCheck(tipoD, ruc, "M", docu, 1,out xd, out msg))
            {
                return msg;
            }
            return string.Empty;
        ...
        public static IEnumerable<Tuple<string, string,decimal>> RetencionImpuestos( string tipo = null)
        {
            var xsalida = new HashSet<Tuple<string, string, decimal>>();
            var con = Conection("midle");
            if (con == null)
            {
                xsalida.Add(Tuple.Create("0", "Catalogo vacío [get]",decimal.Parse("1000")));
                return xsalida;
            }
            using (con)
            {
                ...
                com.CommandType = CommandType.StoredProcedure;
                com.CommandTimeout = 120;
                com.CommandText = "pc_impuestos_ret";
                try
                {
                    con.Open();
                    using (var reader = com.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            xsalida.Add(Tuple.Create(reader.GetString(0), reader.GetString(1),reader.GetDecimal(2)));
                        }
                    }
                    con.Close();
                    com.Dispose();
                ...
                catch (SqlException ex)
                {
                    ...
                    log_csl.save_log<Exception>(new Exception(sb.ToString()), "dataServiceHelper", "RetencionImpuestos", "null", "N4");
                    xsalida.Add(Tuple.Create("0", "Catalogo incompleto [set]",decimal.Parse("1000")));
                    return xsalida;
                }
                finally
                {
