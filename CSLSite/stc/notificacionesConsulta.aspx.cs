...
namespace CSLSite
{
    public partial class notificacionesConsulta : System.Web.UI.Page
    {
        #region "Variables"
        ...
        private void Carga_Inicial_Contenedores()
        {
            try
            {
                ...
                //ClsUsuario.ruc
                List<Lista_Contenedores> TablaCasos = Lista_Contenedores.Listado_Contenedores_AppCgsa(ClsUsuario.ruc, out OError);
                if (TablaCasos != null)
                {
                    var LinqResumen = (from Tbl in TablaCasos.Where(Tbl => Tbl.Container != string.Empty).OrderBy(p=>p.event_date).ThenByDescending(p => p.event_date)
                                       select new
                                        {
                                            UnitGkey = Tbl.UnitGkey,
                                            Container = Tbl.Container,
                                            categoria = Tbl.categoria,
                                        }).Distinct();

                    var LinqDetalle = (from Tbl in TablaCasos.Where(Tbl => Tbl.Container != string.Empty).OrderBy(p => p.event_date).ThenByDescending(p => p.event_date)
                                       select new
                                       {
                                           UnitGkey = Tbl.UnitGkey,
                                           Container = Tbl.Container,
                                           categoria = Tbl.categoria,
                                           idNotificacion = Tbl.idNotificacion,
                                           Name = Tbl.Name,
                                           cuerpo = Tbl.cuerpo,
                                           cert_secuencia = Tbl.cert_secuencia,
                                           id_certificado_carbo = Tbl.id_certificado_carbo,
                                           event_date = Tbl.event_date,
                                           tiene_imagen = Tbl.tiene_imagen
                                       }).Distinct();
                    ...
                    foreach (var Det in LinqResumen)
                    {
                        ...
                        if (LinqDetalle != null)
                        {
                            foreach (var Eve in LinqDetalle.Where(p=> p.UnitGkey==Det.UnitGkey))
                            {
                                ...
                                if (Eve.tiene_imagen)
                                {
                                    ...
                                    v_detalle = v_detalle + " &nbsp &nbsp &nbsp &nbsp <input type='button' name='e' onserverclick='BtnCargarImagenes_Click' value='Ver Imágenes' id='{X2}'  class='btn btn-secondary' data-toggle='modal' data-target='#myModal2'  onclick=clickaction({X2})>";
                                    v_detalle = v_detalle + "</div>";
                                }
                               ...
                               // v_detalle = v_detalle + "<div class=\"row\">";
                                ...
                                //v_detalle = v_detalle + "<div class=\"col-sm-5\"><img src=\"" + "https://apps.cgsa.com.ec/Terminal/img_appcgsa/app_receptio/2021/10/e571c6501bb947399c425777585d9129.JPG" + "\" class=\"img-fluid mb-2\" alt=\"CGSApp\" /></div>";
                                //v_detalle = v_detalle + "</div>";
                                ...
                                v_detalle = v_detalle.Replace("{X2}", Eve.idNotificacion.ToString());
        ...
        protected void BtnBuscar_Click(object sender, EventArgs e)
        {
            ...
            if (Response.IsClientConnected)
            {
                try
                {
                    ...
                    List<Lista_Contenedores> TablaCasos = Lista_Contenedores.Listado_Contenedores_Filtro_AppCgsa(ClsUsuario.ruc, fechadesde,fechahasta, (string.IsNullOrEmpty(this.txtcontenedor.Text) ? null : this.txtcontenedor.Text), out OError);
                    if (TablaCasos != null)
                    {
                        var LinqResumen = (from Tbl in TablaCasos.Where(Tbl => Tbl.Container != string.Empty).OrderBy(p => p.event_date).ThenByDescending(p => p.event_date)
                                           select new
...
///////                    v_detalle = "<div class='room-box'>";
        ...
        protected void BtnCargarImagenes_Click(object sender, EventArgs e)
        {
            ...
            var NotificacionSeleccionada = this.Idnot.Value; //txtContainers.Text.ToString();

            List<Lista_Contenedores> Detalle = Lista_Contenedores.Listado_Eventos_AppCgsa((NotificacionSeleccionada==null ? 0 : Int64.Parse(NotificacionSeleccionada.ToString())), out OError);
            if (Detalle != null)
            {
                foreach (var Det in Detalle)
                {
                    ...
                    if (v_contador == 0)
                    {
                        if (!string.IsNullOrEmpty(Det.Photo1))
                        {
                            v_divImagenes += @"<div class='carousel-item active'>
                                                  <img src = '" + Det.Photo1 + @"' class='d-block w-100' style='height:750px; overflow:auto' alt='...'/>
                        ...
                        if (!string.IsNullOrEmpty(Det.Photo2))
                        {
                            v_divImagenes += @"<div class='carousel-item'>
                                                  <img src = '" + Det.Photo2 + @"' class='d-block w-100' style='height:750px; overflow:auto' alt='...'/>
                                                <div class='carousel-caption d-none d-md-block'>
                                                    <!-- <h5>Second slide label</h5>
                                                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>-->
                                                </div>
                        ...
                        if (!string.IsNullOrEmpty(Det.Photo3))
                        {
                            v_divImagenes += @"<div class='carousel-item'>
                                                  <img src = '" + Det.Photo3 + @"' class='d-block w-100' style='height:750px; overflow:auto' alt='...'/>
                                                <div class='carousel-caption d-none d-md-block'>
                                                    <!-- <h5>Second slide label</h5>
                                                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>-->
                                                </div>
                        ...
                        if (!string.IsNullOrEmpty(Det.Photo4))
                        {
                            v_divImagenes += @"<div class='carousel-item'>
                                                  <img src = '" + Det.Photo4 + @"' class='d-block w-100' style='height:750px; overflow:auto' alt='...'/>
                                                <div class='carousel-caption d-none d-md-block'>
                                                    <!-- <h5>Second slide label</h5>
                                                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>-->
                                                </div>
