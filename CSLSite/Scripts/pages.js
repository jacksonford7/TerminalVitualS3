//2020--->Modernizar mensajes
//2020--->Eliminar los mensajes en amarillo
//nuevo mensajero  alertify.alert('CGA',  ).set('label', 'Aceptar'); 
...
function cedValidate(control, validador) {
    try {
        ...
        if (numced.length <= 0) {
            validator.innerHTML = '<span class="validacion"> *</span>';
                        return false;
        }
...
function cadenareqerida(caja, min, max, validator) {
    ...
    try {
        mens.innerHTML = '';
        if (xcaja == undefined) {
            alertify.alert('El control:[ ' + caja.id + '] no existe');
            return false;
        }
        ...
        if (n == 0) {
            mens.innerHTML = '<span class="validacion"> *</span>';
            return false;
        }
        if (n >= min && n <= max) {
            mens.innerHTML = '';
...
function valDate(caja, requerido, validator) {
    try {
        var xcaja = document.getElementById(caja.id);
        if (xcaja == undefined) {
            alertify.alert('El control:[ ' + xcaja + '] no existe');
            return false;
        }
        if (requerido) {
            if (xcaja.value.trim().length <= 0) {
...
function rulevalidate(control, validador) {
    ...
    validador.innerHTML = '<span class="validacion"> *</span>';
    return;
}
...
function ValidateFile(control, validador) {
    try {
        ...
        if (!(fileName.lastIndexOf(atributo) == fileName.length - 3)) {
            if (b) {
                alert('El archivo excel deber ser guardado como ' + atributo + ' (delimitado por comas)');
                return false;
            }
            validador.innerHTML = '<span class="obligado">El archivo excel deber ser guardado como [' + atributo + ']</span>';
            return false
        }
        if (!b) {
        ...
        try {
            ...
            switch (txt) {
                ...
                case 'DTAI':
                    ...
                    dae2.value = '';
                    dae3.value = '';
                    document.getElementById(ms).textContent = 'D.T.A.I: Declaración de Tránsito Aduanero';
                    dae3.setAttribute('maxlength', 40);
                    break;
                default:
                    //valor default
                    dae1.disabled = false;
...
function validateDatesRange(desde, hasta,ctrl) {
        ...
        if (desde == undefined || desde == null || hasta == undefined || hasta == null) {
            ...
            alertify.alert('CGSA', 'Revise/escriba la fechas').set('label', 'Aceptar'); 
            return false;
        }
        ...
        if (!date instanceof Date || isNaN(date.valueOf())) {
            ...
            alertify.alert('CGSA', 'Revise/escriba la fecha desde').set('label', 'Aceptar'); 
            return false;
        }
        ...
        if (!date2 instanceof Date || isNaN(date2.valueOf())) {
            alertify.alert('CGSA', 'Revise/escriba la fecha hasta').set('label', 'Aceptar'); 
            return false;
        }
        if (date > date2) {

            alertify.alert('CGSA','Rango de fechas está incorrecto,' ).set('label', 'Aceptar'); 
            return false;

        }
        if ((date.getFullYear() - date2.getFullYear()) != 0) {

            alertify.alert('CGSA', 'Rango máximo de busqueda es 1 año').set('label', 'Aceptar'); 
            return false;
        }

        var mini = parseInt(datePart[1]);  var mfin = parseInt(datePart2[1]);
        if (mini != mfin) {
            alertify.alert('CGSA','Rango máximo de busqueda es 1 mes' ).set('label', 'Aceptar'); 
            return false;
        }
...
function linkbokin(clinea, cexport, tipo) {
        try {
            ...
            if (exportador == null || exportador == undefined || exportador == '') {
                alertify.alert('CGSA','Escoja el exportador' ).set('label', 'Aceptar'); 
                return;
            }
            if (linea == null || linea == undefined || linea == '') {
                alertify.alert('CGSA',  'Escoja la línea naviera').set('label', 'Aceptar'); 
                return;
            }
            var w = window.open('../catalogo/booking.aspx?linea=' + linea + '&tipo=' + tipo, 'Bookings', 'width=850,height=900');
            w.focus();
...
function radio(control, control2) {
        ...
        if (txt) {
            $('.xalter').html('<span class="validacion"> *</span>');
    ...
    try {
        ...
        $.ajax({
            ...
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                alertify.alert('ERROR',textStatus + ": " + XMLHttpRequest.responseText);
            }
        });
...
function setRefer(estado) {
        ...
        //2019--ref
        document.getElementById('dprefservice').disabled = estado;
        document.getElementById('dprefservice').selectedIndex = "0";
        if (!estado) {
            document.getElementById('valtemp').innerHTML = '<span class="validacion"> *</span>';
            document.getElementById('valrefri').innerHTML = '<span class="validacion"> *</span>';
...
function getPrint(jAisv,url) {
        if (jAisv.bnumber == undefined || jAisv.bnumber.trim().length <= 0) {
            alertify.alert('CGSA', 'Seleccione el Booking').set('label', 'Aceptar'); 
            window.scrollTo(0, 0);
...
function processOk(response) {
    ...
    if (!jmsg.resultado) {
        alertify.alert('CGSA', jmsg.mensaje ).set('label', 'Aceptar'); 
        if (jmsg.fluir) {
            eval(jmsg.data);
        }
        document.getElementById("loader").className = 'nover';
        ...
        alertify.alert('CGSA', 'Se va a imprimir el AISV No. [' + jmsg.data + ']').set('label', 'Aceptar'); 
...
function addrow() {
    ...
    //alert(xrowcounter);
    if (xrowcounter > 70) {
        alertify.alert('CGSA','Ha llegado al limite de documentos (70 por contenedor)' ).set('label', 'Aceptar'); 
        return;
    }
    ...
    if (dae1 == null || dae2 == null || dae3 == null || dae1 == undefined || dae2 == undefined || dae3 == undefined) {
        alertify.alert('CGSA', 'Escriba el número de documento').set('label', 'Aceptar'); 
        return;
    }
   ...
   // alert(lista);
    for (var i = 0; i < lista.length; i++) {
        if (lista[i].adudoc == documento) {
            alertify.alert('CGSA','Número de documento ya está registrado' ).set('label', 'Aceptar'); 
            return;
        }
    }
    //bultos
...
function checkDC(control, validador, opcional) {
    try {
        ...
        if (codigo.length <= 0) {
            document.getElementById(validador).innerHTML = '<span class="validacion"> *</span>';
            return false;
        }
        if (codigo.length != 11) {
        ...
        if (newverfic != verificador) {
            document.getElementById(validador).innerHTML = '<span class="obligado">Verificador INCORRECTO:[' + verificador + '] -> [' + newverfic + '] </span>';
            return;
        }
...
function oCedulaValida(control, validador) {
    try {
...
function cedulaWarning(valor) {
    try {

        if (valor == null || valor == undefined) {
            return false;
        }
        var numced = valor.trim().toUpperCase();
        var verficador = numced.substring(numced.length - 1);
        if (numced.length <= 0) {
            return false;
        }
...
//nuevo-->2017-->validarDae
function DaeCkecker(OnSuccess, OnError) {
    //obtener parametros
    var tipo_documento = document.getElementById('dpdoc').value;
    var ruc = document.getElementById('ruc_dae').value;
    var dae1 = document.getElementById('txtdae1');
    var dae2 = document.getElementById('txtdae2');
    var dae3 = document.getElementById('txtdae3');
    var documento = dae1.value + dae2.value + dae3.value;
    var params = {};
    params.tipoD = tipo_documento;
    params.doc = documento;
    params.ruc = ruc;
    params = JSON.stringify(params);
    $.ajax({
        type: "POST",
        url: "../services/catalogo.asmx/ValidarDocumento",
        data: params,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
        success: function (response) { OnSuccess(response.d); },
        error: function (e) { OnError(e.message); }
    });
}
function exito(cadena) {
    //aqui tengo la cadena si la cadena es vacía AÑADIR--->
    //si la cadena no es vacía mostrar error.
    if (cadena) {
        //la cadena NO esta vacía MOSTRAR ERROR
        alertify.alert('CGSA',cadena ).set('label', 'Aceptar'); 
        return;
    }
    // ACA LA CADENA ES VACÍA
    addrow();
}
function problema(cadena) {
    //no añadir presentar problema.
    alertify.alert('CGSA',cadena  ).set('label', 'Aceptar'); 
    return;
}