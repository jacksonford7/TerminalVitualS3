...
namespace CSLSite
{
    public class turnoConsolidacion
    {
        ...
        public static DataTable GetBookingsRef(string bookin, string ruc, string referencia)
        {
            var d = new DataTable();
            using (var c = conexion())
            {
                var coman = c.CreateCommand();
                coman.CommandType = CommandType.StoredProcedure;
                coman.CommandText = "n4m_sp_get_booking_ref";
                coman.Parameters.AddWithValue("@booking", bookin);
                coman.Parameters.AddWithValue("@ruc", ruc);
                coman.Parameters.AddWithValue("@refer", referencia);
                try
                {
                    c.Open();
                    d.Load(coman.ExecuteReader(CommandBehavior.CloseConnection));
                ...
                {
                    csl_log.log_csl.save_log<SqlException>(ex, "pago terc", "GetBookings", bookin, "Sistema_to_SQL");
                }
                finally
                {
                    if (c.State == ConnectionState.Open)
