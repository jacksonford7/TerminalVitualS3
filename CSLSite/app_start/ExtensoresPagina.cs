...
namespace CSLSite
{
    ...
    {
        private static string jsarguments(object idServicio, object idFiltro)
        {
        ...
        public static string Opciones(this System.Web.UI.Page page, usuario user, ref HashSet<string> p_lista)
        {
            ...
            if (user != null)
            {
                ...
                div.AppendFormat("<ul class=\"nav flex-column\">");
                div.AppendFormat("<li class=\"nav-item my-1\">");
                div.AppendFormat("<a class=\"nav-link house\" href=\"../cuenta/menu.aspx\">");
                div.AppendFormat("Inicio");
                div.AppendFormat("</a>");
                div.AppendFormat("</li>");
                ...
                foreach (var acceso in user.autorized_zones())
                {
                    ...
                    string NewZona = string.Empty;
                    Zona = acceso.titulo.Trim().Replace("&nbsp;", "");
                    NewZona = Zona.Trim();

                    string cId = QuerySegura.EncryptQueryString(acceso.idservicio.Value.ToString());
                    string IdFiltro = QuerySegura.EncryptQueryString("0");
                    switch (NewZona)
                    {
                        case "Mis Naves":
                            div.AppendFormat("<li class=\"nav-item my-1\"><a class=\"nav-link boat\" href=\"../cuenta/subopciones.aspx?opcion={0}\">{1}</a></li>", jsarguments(cId, IdFiltro), NewZona);
                            break;
                        case "Importación":
                            div.AppendFormat("<li class=\"nav-item my-1\"><a class=\"nav-link import\" href=\"../cuenta/subopciones.aspx?opcion={0}\">{1}</a></li>", jsarguments(cId, IdFiltro), NewZona);
                            break;
                        case "Exportación":
                            div.AppendFormat("<li class=\"nav-item my-1\"><a class=\"nav-link export\" href=\"../cuenta/subopciones.aspx?opcion={0}\">{1}</a></li>", jsarguments(cId, IdFiltro), NewZona);
                            break;
                        case "Depósito de Vacíos":
                            div.AppendFormat("<li class=\"nav-item my-1\"><a class=\"nav-link empty\" href=\"../cuenta/subopciones.aspx?opcion={0}\">{1}</a></li>", jsarguments(cId, IdFiltro), NewZona);
                            break;
                        case "Gestión Financiera":
                            div.AppendFormat("<li class=\"nav-item my-1\"><a class=\"nav-link financial\" href=\"../cuenta/subopciones.aspx?opcion={0}\">{1}</a></li>", jsarguments(cId, IdFiltro), NewZona);
                            break;
                        case "Gestión de Accesos":
                            div.AppendFormat("<li class=\"nav-item my-1\"><a class=\"nav-link keys\" href=\"../cuenta/subopciones.aspx?opcion={0}\">{1}</a></li>", jsarguments(cId, IdFiltro), NewZona);
                            break;
                        case "KPI":
                            div.AppendFormat("<li class=\"nav-item my-1\"><a class=\"nav-link kpi\" href=\"../cuenta/subopciones.aspx?opcion={0}\">{1}</a></li>", jsarguments(cId, IdFiltro), NewZona);
                            break;
                        case "Ajustes":
                            div.AppendFormat("<li class=\"nav-item my-1\"><a class=\"nav-link setup\" href=\"../cuenta/subopciones.aspx?opcion={0}\">{1}</a></li>", jsarguments(cId, IdFiltro), NewZona);
                            break;
                        case "Servicio al Cliente":
                            div.AppendFormat("<li class=\"nav-item my-1\"><a class=\"nav-link headset\" href=\"../cuenta/subopciones.aspx?opcion={0}\">{1}</a></li>", jsarguments(cId, IdFiltro), NewZona);
                            break;
                        case "CGSApp":
                            div.AppendFormat("<li class=\"nav-item my-1\"><a class=\"nav-link appcgsa\" href=\"../cuenta/subopciones.aspx?opcion={0}\">{1}</a></li>", jsarguments(cId, IdFiltro), NewZona);
                            break;
                        case "Port To Door":
                            div.AppendFormat("<li class=\"nav-item my-1\"><a class=\"nav-link p2d\" href=\"../cuenta/subopciones.aspx?opcion={0}\">{1}</a></li>", jsarguments(cId, IdFiltro), NewZona);

                            break;
                        default:
                           // div.AppendFormat("<li class=\"nav-item my-1\"><a class=\"nav-link p2d\" href=\"../cuenta/subopciones.aspx?opcion={0}\">{1}</a></li>", jsarguments(cId, IdFiltro), "Port To Door");

                            //div.AppendFormat("<li class=\"nav-item my-1\"><a class=\"nav-link house\" href=\"../menu/subopciones.aspx?opcion={0}\">{1}</a></li>", cId, NewZona);
                            break;
                    }
            ...
            else
            {
                div.AppendFormat("<ul class=\"nav flex-column\">");
                div.AppendFormat("<li class=\"nav-item my-1\">");
                div.AppendFormat("<a class=\"nav-link house\" href=\"../menu/menu.aspx\">");
                div.AppendFormat("Inicio");
                div.AppendFormat("</a>");
                div.AppendFormat("</li>");
                div.AppendFormat("</ul>");
        ...
        public static string Opciones2(this System.Web.UI.Page page, usuario user, ref HashSet<string> p_lista)
        {
            int id_Servicio = 0;
            int id_opcion = 0;
        ...
        public static void SslOn(this System.Web.UI.Page page)
        {
            if (!HttpContext.Current.Request.IsSecureConnection)
            {
                ...
                if (valor == null || valor.Contains("0"))
                {
                    ...
                    page.Response.Write(string.Format("<img src='{0}'/><hr/>Terminal Virtual<br/><div>Por su protección se ha denegado el acceso, por favor cierre esta ventana y luego intente abrir sesión nuevamente.</div><hr/>", VirtualPathUtility.ToAbsolute("~/shared/imgs/logocontecon.png")));
            ...
            for (int i = 0; i <= parametros.Length - 1; i++)
            {
                if (qs)
                {
                    if (HttpContext.Current.Request.QueryString[parametros[i]] == null)
                    {
                        ...
                        page.Response.Write(string.Format("<img src='{0}'/><hr/>Terminal Virtual<br/><div>Hubo un problema durante la solicitud por favor reintente en unos minutos..</div><hr/>", VirtualPathUtility.ToAbsolute("~/shared/imgs/logocontecon.png")));
                        page.Response.Write(close);
                        page.Response.End();
                        return;
                    }
                ...
                else
                {
                    if (page.RouteData.Values[parametros[i]] == null)
                    {
                        ...
                        string close = string.Format(@"<script type='text/javascript'>alert('{0}'); window.returnValue = true; window.close();</script>", mensaje);
                        csl_log.log_csl.save_log<ApplicationException>(new ApplicationException(string.Format("IP:{0},URL:{1},METODO:{2}, No se encontó una variable en el QS", HttpContext.Current.Request.UserHostAddress, HttpContext.Current.Request.Url, HttpContext.Current.Request.HttpMethod)), page.GetType().ToString(), "ReviewQS", parametros[i], "Espía");
                        page.Response.Write("<img src='../shared/imgs/logocontecon.png'/><hr/>Terminal Virtual<br/><div>Hubo un problema durante la solicitud por favor reintente en unos minutos..</div><hr/>");
                        page.Response.Write(close);
                        page.Response.End();
                        return;
                    }
        ...
        public static void IsJSSuport(this System.Web.UI.Page page)
        {
            ...
            if (!(HttpContext.Current.Request.Browser.EcmaScriptVersion.Major < 1))
            {
                ...
                page.Response.Write(string.Format("<img src='{0}'/><hr/>Terminal Virtual<br/><div>Para que el Sistema funcione correctamente su navegador debe soportar Javascript, gracias por entender.<br/>Para mas información comuníquese con Contecon, área Desarrollo.</div><hr/>", VirtualPathUtility.ToAbsolute("~/shared/imgs/logocontecon.png")));
                page.Response.End();
                return;
            }
        }
        public static void PersonalResponse(this System.Web.UI.Page page,string mensaje,string newurl = null, bool redirect=false)
        {
            ...
            response = string.Format("<img src='{1}'/><hr/>Terminal Virtual<br/><div>{0}</div><hr/>", mensaje, VirtualPathUtility.ToAbsolute("~/shared/imgs/logocontecon.png"));
            if (redirect)
            {
                response = string.Concat(string.Format("<meta http-equiv=\"refresh\" content=\"10; url={0}\" />",newurl),response);
            }
            page.Response.Write(response);
            page.Response.End();

        }
        public static void AbortResponse(this System.Web.UI.Page page, string mensaje, string newurl = null, bool redirect = false)
        {
            page.Response.Clear();
            var response = string.Empty;
            response = string.Format("<img src='{1}'/><hr/>Terminal Virtual<br/><div>{0}</div><hr/>", mensaje, VirtualPathUtility.ToAbsolute("~/shared/imgs/logocontecon.png"));
        ...
        public static void IsCompatibleBrowser(this System.Web.UI.Page page )
        {
            ...
            {
                var st = string.Empty;
                st = string.Concat(st, string.Format("<img src='{0}'/><hr/>Terminal Virtual<br/><div>Para el correcto funcionamiento del Sistema, necesita un navegador con una versión superior a la que usted posee instalado:</div><hr/>", VirtualPathUtility.ToAbsolute("~/shared/imgs/logocontecon.png")));
        ...
        public static void IsAllowAccess(this System.Web.UI.Page page)
        {
            ...
            string baseUrl = page.Request.Url.GetLeftPart(UriPartial.Authority);
            ...
            if (HttpContext.Current.Session["acceso"] == null)
            {
                ...
                PersonalResponse(page, string.Format("La sesión de usuario ha expirado, será redireccionado a la pantalla de inicio. Ticket [{0}]",ticket), baseUrl+ "../login.aspx", true);
                return;
            }
            ...
            var t = page.Request.RawUrl.Split('?').FirstOrDefault();
            if (!string.IsNullOrEmpty(t))
            {
                t = t.Split('/').LastOrDefault();
            }

            //tus opciones vienen con "? Y UN STRING Encriptado esto lo arregla"
            if (!opt.Contains(t))
            {
        ...
        public static void Alerta(this System.Web.UI.Page page, string mensaje, bool exit=false)
        {
            ...
            if (!string.IsNullOrEmpty(mensaje))
            {
                if (AjaxControlToolkit.ToolkitScriptManager.GetCurrent(page) != null)
                {
                    ...
                    {
                        sb.AppendFormat(" alertify.alert('CGSA', '{0}').set('label', 'Aceptar');", mensaje);
                         if (exit)
                        {
        ...
        public static void Popup(this System.Web.UI.Page page, string url)
        {
            ...
            if (!string.IsNullOrEmpty(url))
            {
                if (AjaxControlToolkit.ToolkitScriptManager.GetCurrent(page) != null)
                {
                    if (AjaxControlToolkit.ToolkitScriptManager.GetCurrent(page).IsInAsyncPostBack)
                    {
                        sb.AppendFormat("window.open('{0}','Impresion','width=850,height=680,scrollbars=yes,resizable=yes');", url);
                        System.Web.UI.ScriptManager.RegisterStartupScript(page, page.GetType(), (new Guid()).ToString(), sb.ToString(), true);
                    }
                    else
                    {
                ...
                if (AjaxControlToolkit.ToolkitScriptManager.GetCurrent(page) != null)
                {
                    ...
                    else
                    {
                      ...
                      //  sb.AppendFormat("alert('{0}');", script);
                        sb.AppendFormat("alertify.alert('CGSA', {0} ).set('label', 'Aceptar');", script);
