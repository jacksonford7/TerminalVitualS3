...
namespace CSLSite.app_start
{
    public class ProformaContado
    {
        ...
        public string chofer { get; set; } //pedir
        public string placa { get; set; } //pedir
        ...
        public ProformaContado()
        {
            this.Cantidad = 0;
            this.Estado = true;
            this.Detalle = new HashSet<ProformaContadoDetalle>();
            this.IdCorporacion = 1;
            this.IdEmpresa = 1;
            this.IdServicio = 33;
            this.Actividad = "E";
            this.FechaSalida = DateTime.Now;
            this.Detalle_Turnos = new HashSet<DetalleTurnos>();
        }

        public HashSet<ProformaContadoDetalle> Detalle { get; set; }

        public HashSet<DetalleTurnos> Detalle_Turnos { get; set; }
            ...
            using (var con = new SqlConnection())
            {
                ...
                using (var tx = con.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (user.IsCredito)
                        {
                            email.AppendFormat("<h2>Se ha generado exitosamente la Proforma Por Reserva de Turno No. {0}</h2>", secuencia.Value);
                        }
                        else
                        {
                            email.AppendFormat("<h2>Se ha generado exitosamente la Proforma Por Reserva de Turno No. {0}</h2>", secuenciaContado);
                        }
                        email.Append("<h3>A continuación el detalle:</h3>");
                        email.AppendFormat("<h4>Booking Number.{0}</h4>", Bokingnbr);
                        email.AppendFormat("<p>Turnos reservados: {0} (cantidad seleccionada por el cliente)</p>", Cantidad);

                        foreach (var tt in this.Detalle_Turnos)
                        {
                            email.AppendFormat("<p>Fecha y Hora de Emisión: {0}</p>", tt.fecha);
                            email.AppendFormat("<p>Chofer: {0}</p>", tt.chofer);
                            email.AppendFormat("<p>Placa: {0}</p>", tt.placa);
                            email.Append("<p>--------------------------------------------------------------------------------</p>");
                        }
                        /*
                        email.AppendFormat("<p>Fecha y Hora de Emisión: {0}</p>", DateTime.Now.ToString("yyyy-MM-dd HH:mm"));
                        email.AppendFormat("<p>Chofer: {0}</p>", this.chofer);
                        email.AppendFormat("<p>Placa: {0}</p>", this.placa);
                        */
                        ...
                        email.AppendFormat("<tr><td colspan='4' style='text-aling:right;'><strong>SUBTOTAL</strong></td><td valign='top'>{0:c}</td></tr>",this.SubTotal);
                        email.AppendFormat("<tr><td colspan='4' style='text-aling:right;'><strong>IVA</strong></td><td valign='top'>{0:c}</td></tr>", this.IVA);
                        email.AppendFormat("<tr><td colspan='4' style='text-aling:right;'><strong>NETO A PAGAR</strong></td><td>{0:c}</td></tr>", Total);
                        email.Append("</table>");
                        email.AppendFormat("<p><strong>Importante: </strong>Recuerde que la liquidaci&oacute;n debe estar pagada  hasta 2 horas antes del turno <br>seleccionado para que se active y pueda ser  utilizado.</p>");
                        ...
                        mcom.Parameters.AddWithValue("@asunto", string.Format("SNA-ZAL:  Servicio de Notificaciones Automáticas -  Proforma Por Reserva de Turno del Booking No. {0}", Bokingnbr));
    ...
    public class ProformaContadoDetalle
    {
        ...
        public ProformaContadoDetalle() { this.TarifaControl = 1; this.IdContenedor = 1; this.Tipo = "PO"; }
    ...
    public class DetalleTurnos
    {
        ...
        public string fecha { get; set; }//si
        public string chofer { get; set; }//si
        public string placa { get; set; }
       
        public DetalleTurnos() { this.fecha = ""; this.chofer = ""; this.placa = ""; }
