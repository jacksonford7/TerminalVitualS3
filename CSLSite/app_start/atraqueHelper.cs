...
namespace CSLSite
{
    public class atraqueHelper
    {
        ...
        //NUEVO 2018---> VARIAS  CIA DE SEGUROS
        public static List<item> get_seguros_cia()
        {
            var xsalida = new List<item>();
            var con = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["service"].ConnectionString);
            if (con == null)
            {
                return xsalida;
            }
            using (con)
            {
                ...
                com.CommandType = CommandType.StoredProcedure;
                com.CommandTimeout = 120;
                com.CommandText = "[dbo].[sp_get_compania_seguridad]";
                try
                {
                    con.Open();
                    using (var reader = com.ExecuteReader())
                ...
                catch (SqlException ex)
                {
                    ...
                    log_csl.save_log<Exception>(new Exception(sb.ToString()), "atraqueHelper", "get_seguros_cia", "TODOS", "N4");
                    return xsalida;
                }
                finally
                {
