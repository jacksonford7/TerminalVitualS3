...
namespace SqlConexion
{
    public class Cls_Conexion
    {
        ...
        private Cls_Conexion()
        {
            ...
            if (string.IsNullOrEmpty(conexion_Local) && System.Configuration.ConfigurationManager.ConnectionStrings["billion"] != null)
            {
                conexion_Local = System.Configuration.ConfigurationManager.ConnectionStrings["billion"].ConnectionString;
                var s = TestServer();
            ...
            {
                throw new ApplicationException("La cadena de conexión billion, no esta presente en el archivo de configuración");
            }
        ...
        public T ExecuteSelectOnlyValue<T>(string conection, int timeout, string pc_name, Dictionary<string, object> parameters)
        {
            using (var sql = new SqlConnection(conection))
            {
                using (var sp_com = new SqlCommand(pc_name, sql))
                {
                    sp_com.CommandType = CommandType.StoredProcedure;
                    sp_com.CommandTimeout = timeout;
                    ...
                    {
                        sql.Open();
                        var result = sp_com.ExecuteScalar();
