...
namespace BreakBulk
{
    public class ubicacion : Cls_Bil_Base
    {
        #region "Propiedades"
        ...
        public static DataView consultaUbicacionparaTurnosConsolidacion()
        {
            OnInit("VBS");
            SqlConnection cn = new SqlConnection(nueva_conexion);
            ...
            using (var c = cn)
            {
                var coman = c.CreateCommand();
                coman.CommandType = CommandType.StoredProcedure;
                coman.CommandText = "VBS_TURNOS_CONSOLIDACION_UBICACION_CONSULTAR";
                try
                {
                    c.Open();
                    d.Load(coman.ExecuteReader(CommandBehavior.CloseConnection));
