...
namespace CSLSite
{
    ...
    public class CLSDataCentroSolicitud
    {
                    ...
                    try
                    {
                        ...
                        using (var tx = xcon.BeginTransaction())
                        {
                            ...
                            catch (SqlException ex)
                            {
                                ...
                                var t = log_csl.save_log<SqlException>(ex, "CLSDataCentroSolicitud", "add-Trx", idlinea, linea);
            ...
            tComando tipo, tConexion conexion)
        {
            ...
            try
            {
                using (var con = getCon(conexion))
                {
                    ...
                    catch (SqlException ex)
                    {
                        ...
                        log_csl.save_log<ApplicationException>(new ApplicationException(errlis.ToString()), "CLSData", "excuteOnqueryInOut", comandtext, "S3");
                        return false;
                    }
                    finally
                    {
            ...
            {
                log_csl.save_log<Exception>(ex, "CLSData", "excuteOnqueryInOut", comandtext, "S3");
                return false;
            }
        }
        ...
        public static bool crearIIE(Int64 ikey, string ibooking,string iuser, string idae, out string transaccionm)
        {
            ...
            try
            {
                using (var con = getCon(tConexion.Ecuapass))
                {
                    ...
                    catch (SqlException ex)
                    {
                        ...
                        log_csl.save_log<ApplicationException>(new ApplicationException(errlis.ToString()), "CLSData", "crearIIE", ikey.ToString(), "S3");
                        return false;
                    }
                    finally
                    {
            ...
            {
                log_csl.save_log<Exception>(ex, "CLSData", "crearIIE", ikey.ToString(), "S3");
                transaccionm = "Hubo un problema de comunicación favor intente mas tarde";
                return false;
            }
        }
