...
{
    [Serializable]
    [XmlRoot(ElementName = "container_cfs")]
    public class container_cfs : ModuloBase
    {
        ...
        public Int64? CNTR_PESO { get; set; }
        ...
        public ResultadoOperacion<List<container_cfs>> CargaPorBL(string usuario, string mrn,string msn, string hsn)
        {
            ...
            {
                return Respuesta.ResultadoOperacion<List<container_cfs>>.CrearFalla(pv);
            }
            //fin inicializa
            ...
            {
                return Respuesta.ResultadoOperacion<List<container_cfs>>.CrearFalla(string.Format(tt.Item1, nameof(usuario)));
            }
            if (string.IsNullOrEmpty(mrn))
            {
                return Respuesta.ResultadoOperacion<List<container_cfs>>.CrearFalla(string.Format(tt.Item1, nameof(mrn)));
            }
            if (string.IsNullOrEmpty(msn))
            {
                return Respuesta.ResultadoOperacion<List<container_cfs>>.CrearFalla(string.Format(tt.Item1, nameof(msn)));
            }
            ...
            var rsql = BDOpe.ComandoSelectALista<container_cfs>(bcon, "[Bill].[container_cfs_impo]", this.Parametros);
#if DEBUG
            this.LogEvent(usuario, actualMetodo, string.Format("{0}/{1}/{2}",mrn,msn,hsn));
#endif
            return rsql.Exitoso ? ResultadoOperacion<List<container_cfs>>.CrearResultadoExitoso(rsql.Resultado, string.Format("Filas {0}", rsql.Resultado.Count)) : ResultadoOperacion<List<container_cfs>>.CrearFalla(rsql.MensajeProblema);
        }
        ...
        public static ResultadoOperacion<List<Tuple<Int64, string, string, bool>>> ValidarEstadoTransaccion(List<Int64> pases, string usuario)
        {
            ...
            var p = new Importacion.container_cfs();
            ...
            var rsql = BDOpe.ComandoSelectEscalarRef<string>(bcon, "Select [Bill].[transaccion_cfs_estado](@pases);", p.Parametros);
            if (!rsql.Exitoso)
            {
