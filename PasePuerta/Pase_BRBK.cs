...
{
    public class Pase_BRBK : ModuloBase
    {
        ...
        public ResultadoOperacion<Int64> Insertar(
            Int64 ID_PLAN, 
             string CONTENEDOR, int TOTAL_BULTOS, string MRN, string MSN, string HSN,   bool IsDD=false)
        {
            ...
            code.parameters.Add("codsubitem", "");
            code.parameters.Add("placa", ID_PLACA);
            code.parameters.Add("tipo_carga", "BRBK");
            ...
            if (n4r.status != 1)
            {
                ...
                var i =  this.LogError<ApplicationException>(ex,"InsertarPaseBRBK",USUARIO_REGISTRO);
            ...
            var result =  BDOpe.ComandoInsertUpdateDeleteID(bcon, "[Bill].[nuevo_pase_puerta_brbk]", this.Parametros);
        ...
        //debe insertar, debe modificar, debe eliminar
        //Insertar pase a puerta
        public ResultadoOperacion<Int64> Insertar_Solicitud(
            Int64 ID_PLAN,
             string CONTENEDOR, int TOTAL_BULTOS, string MRN, string MSN, string HSN, Int64 ID_SOL, Int64 SECUENCIA_SOL, bool IsDD = false)
        {
            this.actualMetodo = MethodBase.GetCurrentMethod().Name;
            string pv = string.Empty;
            if (!this.Accesorio.Inicializar(out pv))
            ...
            this.Parametros.Add(nameof(TOTAL_BULTOS), TOTAL_BULTOS);
            this.Parametros.Add(nameof(ID_UNIDAD), ID_UNIDAD);
            this.Parametros.Add(nameof(ID_SOL), ID_SOL);
            this.Parametros.Add(nameof(SECUENCIA_SOL), SECUENCIA_SOL);

            var bcon = this.Accesorio.ObtenerConfiguracion("N4Middleware")?.valor;
            var result = BDOpe.ComandoInsertUpdateDeleteID(bcon, "[Bill].[nuevo_pase_puerta_brbk_solicitud]", this.Parametros);
            this.ID_PASE = result.Resultado.HasValue ? result.Resultado.Value : -1;
            if (!result.Exitoso)
            {
                this.LogEvent(USUARIO_REGISTRO, actualMetodo, string.Format("Falló la inserción {0}/{1}/{2}", ID_CARGA, FECHA_EXPIRACION, result.MensajeProblema));
        ...
        public ResultadoOperacion<bool> Cancelar(string USUARIO_CANCELA)
        {
            ...
            if (n4r.status != 1)
            {
                ...
                var ex = new ApplicationException(n4r.status_id);
                var i =  this.LogError<ApplicationException>(ex,"CancelarPaseBRBK",USUARIO_REGISTRO);
            ...
            this.Parametros.Add(nameof(PPW), PPW);
            var bcon = this.Accesorio.ObtenerConfiguracion("N4Middleware")?.valor;
            var result = BDOpe.ComandoInsertUpdateDeleteID(bcon, "[Bill].[cancelar_pase_puerta_brbk]", this.Parametros);
            if (!result.Exitoso)
            {
        ...
        public ResultadoOperacion<bool> Actualizar(string USUARIO_MODIFICA, int? ID_PLAN, string CAMBIO_TURNO, string MRN, string MSN, string HSN)
        {
            //inicializar
            this.actualMetodo = MethodBase.GetCurrentMethod().Name;
            ...
            if (CAMBIO_TURNO.Equals("SI"))
            {
                actualizaturno = true;
            }
            else
            {
                actualizaturno = false;
            }
            ...
            if (actualizaturno)
            {
                ...
                if (n4r.status != 1)
                {
                    ...
                    var ex = new ApplicationException(n4r.status_id);
                    var i = this.LogError<ApplicationException>(ex, "ActualizarBRBK", USUARIO_REGISTRO);
                    var emsg = string.Format("Ha ocurrido la novedad número {0} durante el proceso de creación del pase, el mismo presenta los siguientes problemas:{1}...favor comuníquese con SAC.", i.HasValue ? i.Value : -1, Mensaje_N4);
                    return Respuesta.ResultadoOperacion<bool>.CrearFalla(emsg);
                }
                ...
                code1.parameters.Add("codsubitem","");
                code1.parameters.Add("placa", ID_PLACA);
                code1.parameters.Add("tipo_carga", "BRBK");
            ...
            this.Parametros.Add(nameof(NUMERO_PASE_N4), NUMERO_PASE_N4);
            this.Parametros.Add(nameof(TRANSPORTISTA_DESC), TRANSPORTISTA_DESC);
            this.Parametros.Add(nameof(CAMBIO_TURNO), CAMBIO_TURNO);

            var bcon = this.Accesorio.ObtenerConfiguracion("N4Middleware")?.valor;
             var result =  BDOpe.ComandoInsertUpdateDeleteFila(bcon, "[Bill].[modifica_pase_puerta_brbk]", this.Parametros);
        ...
        public ResultadoOperacion<bool> Actualizar_Transportista(string USUARIO_MODIFICA,  string MRN, string MSN, string HSN)
        {
            //inicializar
            this.actualMetodo = MethodBase.GetCurrentMethod().Name;
            ...
            var bcon = this.Accesorio.ObtenerConfiguracion("N4Middleware")?.valor;
            var result = BDOpe.ComandoInsertUpdateDeleteFila(bcon, "[Bill].[modifica_transporte_pase_puerta_brbk]", this.Parametros);
       ...
       BRBK_MRN=MRN  AND BRBK_MSN=MSN AND BRBK_HSN=HSN
        ...
        public static Respuesta.ResultadoOperacion<System.Data.DataTable> ObtenerListaEditable(string MRN, string MSN, string HSN, string RUC, string USUARIO, DateTime? fecha = null)
        {
            ...
            var bcon = p.Accesorio.ObtenerConfiguracion("N4Middleware")?.valor;
            //OBTENER LA LISTA DE PASES.
            var dtw = BDOpe.ComadoSelectADatatable(bcon, "[Bill].[lista_editar_pase_brbk]", p.Parametros);
            if (!dtw.Exitoso)
            {
        ...
        public static Respuesta.ResultadoOperacion<System.Data.DataTable> ObtenerListaEditable_Transportista(string MRN, string MSN, string HSN, string RUC, string USUARIO, DateTime? fecha = null)
        {
            /*
             [Bill].[lista_editar_pase] 
             */
            ...
            var bcon = p.Accesorio.ObtenerConfiguracion("N4Middleware")?.valor;
            //OBTENER LA LISTA DE PASES.
            var dtw = BDOpe.ComadoSelectADatatable(bcon, "[Bill].[lista_editar_pase_brbk_transporte]", p.Parametros);
            if (!dtw.Exitoso)
            {
                return ResultadoOperacion<System.Data.DataTable>.CrearFalla(dtw.MensajeProblema);
            }
        ...
        public static ResultadoOperacion<bool> Marcar_Servicio(string ID_USUARIO,Int64 UNIDAD_ID, Dictionary<Int64,string> PASES)
        {
          ...
          //  p.Parametros.Add(nameof(ID_USUARIO), ID_USUARIO);
            ...
            //poner el evento
            var n4 = N4Ws.Entidad.Servicios.PonerEventoPasePuertaBRBK(UNIDAD_ID, ID_USUARIO);
            if (!n4.Exitoso)
            {
                p.LogEvent(ID_USUARIO, p.actualMetodo, string.Format("Falló la puesta del servicio Pase_BRBK {0}, RAZON {1}", UNIDAD_ID, n4.MensajeProblema));
                return Respuesta.ResultadoOperacion<bool>.CrearFalla(n4.MensajeProblema);
            }
            var bcon = p.Accesorio.ObtenerConfiguracion("N4Middleware")?.valor;
            var result = BDOpe.ComandoInsertUpdateDeleteFila(bcon, "[Bill].[pase_carga_servicio_brbk]", p.Parametros);
            if (!result.Exitoso)
            {
                p.LogEvent(ID_USUARIO, p.actualMetodo, string.Format("Falló la ACTUALIZACION DEL Pase_BRBK {0} RAZON, {1}", UNIDAD_ID, result.MensajeProblema));
                return Respuesta.ResultadoOperacion<bool>.CrearFalla(result.MensajeProblema);
            }
        ...
        public static Respuesta.ResultadoOperacion<System.Data.DataSet> ImprimirPasesBreakBulk(List<string> pases, string usuario)
        { 
            var p = new Pase_CFS();
            p.actualMetodo = MethodBase.GetCurrentMethod().Name;
            string pv;
            ...
            var bcon = p.Accesorio.ObtenerConfiguracion("N4Middleware")?.valor;
            //OBTENER LA LISTA DE PASES.
            var dtw = BDOpe.ComadoSelectADatatable(bcon, "[Bill].[info_pase_brbk]", p.Parametros);
            if (!dtw.Exitoso)
            {
                return ResultadoOperacion<System.Data.DataSet>.CrearFalla(dtw.MensajeProblema);
            }
            ...
            bcon = p.Accesorio.ObtenerConfiguracion("N5")?.valor;
            var dtp = BDOpe.ComadoSelectADatatable(bcon, "[Bill].[info_pase_brbk_unit]", p.Parametros);
            if (!dtp.Exitoso)
            {
                return ResultadoOperacion<System.Data.DataSet>.CrearFalla(dtp.MensajeProblema);
            }
            ...
            var resultsa = (from table1 in dt1.AsEnumerable() //middleware
                           ...
                           new {
                            ...
                            select   new
                            {
                                CONTAINER = table1["PASE"] as string,
                                MRN =  table1["MRN"] as string,
                                MSN = table1["MSN"] as string,
                                HSN = table1["HSN"] as string,
                                BL = table1["BL"] as string,
        ...
        //debe insertar, debe modificar, debe eliminar
        //Insertar pase a puerta
        public ResultadoOperacion<Int64> Genera_Solicitud(
             string BODEGA, int TIPO_PRODUCTO, string MRN, string MSN, string HSN, string USUARIO_ING, string USUARIO_DESC, 
             string AGENTE_DESC, string IMPORTADOR_DESC, string XML, Int64 ID_TIPO_TURNO)
        {
            this.actualMetodo = MethodBase.GetCurrentMethod().Name;
            string pv = string.Empty;
            if (!this.Accesorio.Inicializar(out pv))
            ...
            this.LogEvent(USUARIO_REGISTRO, actualMetodo, string.Format("Inicia creación de solicitud {0}/{1}", "SIN USUARIO", FECHA_EXPIRACION));
            ...
            this.Parametros.Add(nameof(BODEGA), BODEGA);
            this.Parametros.Add(nameof(TIPO_PRODUCTO), TIPO_PRODUCTO);
            this.Parametros.Add(nameof(USUARIO_ING), USUARIO_ING);
            this.Parametros.Add(nameof(USUARIO_DESC), USUARIO_DESC);
            this.Parametros.Add(nameof(AGENTE_DESC), AGENTE_DESC);
            this.Parametros.Add(nameof(IMPORTADOR_DESC), IMPORTADOR_DESC);
            this.Parametros.Add(nameof(XML), XML);
            this.Parametros.Add(nameof(RUC), RUC);
            this.Parametros.Add(nameof(ID_TIPO_TURNO), ID_TIPO_TURNO);

            var bcon = this.Accesorio.ObtenerConfiguracion("N4Middleware")?.valor;
            var result = BDOpe.ComandoInsertUpdateDeleteID(bcon, "[Bill].[solicitud_turnos_brbk]", this.Parametros);
            this.ID_PASE = result.Resultado.HasValue ? result.Resultado.Value : -1;
            if (!result.Exitoso)
            {
                this.LogEvent(USUARIO_REGISTRO, actualMetodo, string.Format("Falló la inserción {0}/{1}/{2}", string.Format("{0}-{1}-{2}", MRN, MSN, HSN), FECHA_EXPIRACION, result.MensajeProblema));
                return Respuesta.ResultadoOperacion<Int64>.CrearFalla(result.MensajeProblema);
            }
            this.LogEvent(USUARIO_REGISTRO, actualMetodo, string.Format("Termina creación de PP {0}/{1}", string.Format("{0}-{1}-{2}", MRN, MSN, HSN), FECHA_EXPIRACION));
            return Respuesta.ResultadoOperacion<Int64>.CrearResultadoExitoso(result.Resultado.HasValue ? result.Resultado.Value : -1);
        }
