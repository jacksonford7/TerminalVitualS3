...
{
    public class Pase_CFS : ModuloBase
    {
 ...
 public string TRANSPORTISTA_DESC { get; set; }
        ...
        ////cancelar multidespacho
        public ResultadoOperacion<bool> Cancelar_Multidespacho(string USUARIO_CANCELA)
        {
            //pone pase a puerta CA, pone reserva a X, VBS_web a CA
            this.actualMetodo = MethodBase.GetCurrentMethod().Name;
            string pv = string.Empty;
            ...
            this.Parametros.Add(nameof(PPW), PPW);
            var bcon = this.Accesorio.ObtenerConfiguracion("N4Middleware")?.valor;
            var result = BDOpe.ComandoInsertUpdateDeleteID(bcon, "[Bill].[cancelar_pase_puerta_cfs_multidespacho]", this.Parametros);
            if (!result.Exitoso)
            {
                this.LogEvent(USUARIO_REGISTRO, actualMetodo, string.Format("Falló la cancelacion {0}/{1}", ID_PASE, result.MensajeProblema));
                return Respuesta.ResultadoOperacion<bool>.CrearFalla(result.MensajeProblema);
        ...
        ////cancelar multidespacho FREIGHT FORWARDER
        public ResultadoOperacion<bool> Cancelar_MultidespachoTransporte(string USUARIO_CANCELA)
        {
            //pone pase a puerta CA, pone reserva a X, VBS_web a CA
            this.actualMetodo = MethodBase.GetCurrentMethod().Name;
            string pv = string.Empty;
            ...
            this.Parametros.Add(nameof(PPW), PPW);
            var bcon = this.Accesorio.ObtenerConfiguracion("N4Middleware")?.valor;
            var result = BDOpe.ComandoInsertUpdateDeleteID(bcon, "P2D_MULTI_cancelar_pase_puerta_cfs", this.Parametros);
            if (!result.Exitoso)
            {
                this.LogEvent(USUARIO_REGISTRO, actualMetodo, string.Format("Falló la cancelacion {0}/{1}", ID_PASE, result.MensajeProblema));
                return Respuesta.ResultadoOperacion<bool>.CrearFalla(result.MensajeProblema);
       ...
       BRBK_MRN=MRN  AND BRBK_MSN=MSN AND BRBK_HSN=HSN
        ...
        public static Respuesta.ResultadoOperacion<System.Data.DataTable> ObtenerListaEditable_MultiDespacho(string MRN,  string RUC, string USUARIO)
        {
            /*
             [Bill].[lista_editar_pase] 
             */
            ...
            var bcon = p.Accesorio.ObtenerConfiguracion("N4Middleware")?.valor;
            //OBTENER LA LISTA DE PASES.
            var dtw = BDOpe.ComadoSelectADatatable(bcon, "[Bill].[lista_editar_pase_cfs_multidespacho]", p.Parametros);
            if (!dtw.Exitoso)
            {
                return ResultadoOperacion<System.Data.DataTable>.CrearFalla(dtw.MensajeProblema);
            }
        ...
        public static Respuesta.ResultadoOperacion<System.Data.DataTable> ObtenerListaEditable_MultiDespachoTransporte(string MRN, string RUC, string USUARIO)
        {
            /*
             [Bill].[lista_editar_pase] 
             */
            ...
            var bcon = p.Accesorio.ObtenerConfiguracion("N4Middleware")?.valor;
            //OBTENER LA LISTA DE PASES.
            var dtw = BDOpe.ComadoSelectADatatable(bcon, "P2D_MULTI_lista_editar_pase_cfs", p.Parametros);
            if (!dtw.Exitoso)
            {
                return ResultadoOperacion<System.Data.DataTable>.CrearFalla(dtw.MensajeProblema);
            }
        ...
        public static ResultadoOperacion<bool> Marcar_Servicio_MultiDespacho(string ID_USUARIO, Int64 UNIDAD_ID, Dictionary<Int64, string> PASES)
        {
            var p = new Pase_Web();
            p.actualMetodo = MethodBase.GetCurrentMethod().Name;
            string pv;
            ...
            var bcon = p.Accesorio.ObtenerConfiguracion("N4Middleware")?.valor;
            var result = BDOpe.ComandoInsertUpdateDeleteFila(bcon, "[Bill].[pase_carga_servicio_cfs_multidespacho]", p.Parametros);
            if (!result.Exitoso)
            {
                p.LogEvent(ID_USUARIO, p.actualMetodo, string.Format("Falló la ACTUALIZACION DEL PASE_CFS {0} RAZON, {1}", UNIDAD_ID, result.MensajeProblema));
                return Respuesta.ResultadoOperacion<bool>.CrearFalla(result.MensajeProblema);
