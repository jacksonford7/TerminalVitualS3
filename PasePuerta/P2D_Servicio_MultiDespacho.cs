...
{
    public class P2D_Servicio_MultiDespacho : ModuloBase
    {
        ...
        public static ResultadoOperacion<bool> Marcar_Servicio_MultiDespacho_Cfs(string ID_USUARIO, Int64 id_unidades, decimal qty)
        {
            var p = new Pase_Web();
            ...
            var n4 = N4Ws.Entidad.Servicios.PonerEvento_MultiDespacho(id_unidades, ID_USUARIO, qty);
            if (!n4.Exitoso)
            {
                p.LogEvent(ID_USUARIO, p.actualMetodo, string.Format("Falló la puesta del servicio MultiDespacho - Pesos cfs  {0}, usuario {1}", ID_USUARIO, n4.MensajeProblema));
